...
import { DollarSign, Package, ShoppingCart, Zap, ArrowUpRight, ArrowDownRight, Target, Users, LineChart, HelpCircle } from "lucide-react"
...
export function StatsCards({ competitors }: StatsCardsProps) {
  ...
  const avgMatchScore = competitors.length > 0
  ...
  const pricePositioning = competitors.reduce((acc, comp) => {
    const avgCompetitorPrice = comp.products.reduce((sum, p) => sum + (p.price || 0), 0) / (comp.products.length || 1)
    const avgMatchedPrice = comp.products.reduce((sum, p) => {
  ...
  const marketCoverage = competitors.length > 0
    ? Math.round((competitors.filter(c => c.dataGaps.length === 0).length / competitors.length) * 100)
    : 0

  // Get unique platforms
  const uniquePlatforms = new Set(
    competitors
      .filter((c): c is CompetitorBase & { metadata?: { platforms?: { platform: string }[] } } => 'metadata' in c)
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {marketCoverage > 80 ? 'We have great data about your market!' : 'Still gathering more info'}
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="flex items-center">
              <div className="text-2xl font-bold">
                {pricePositioning > 1 ? 'Higher than' : pricePositioning < 1 ? 'Lower than' : 'Same as'} competitors
              </div>
              {pricePositioning !== 1 && (
                <span className={`ml-2 ${pricePositioning > 1 ? 'text-red-500' : 'text-green-500'}`}>
                  {Math.abs((pricePositioning - 1) * 100).toFixed(1)}%
                  {pricePositioning > 1 ? <ArrowUpRight className="h-4 w-4 inline" /> : <ArrowDownRight className="h-4 w-4 inline" />}
                </span>
              )}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {avgMatchScore >= 80 ? "You're leading your market! 🎉" :
               avgMatchScore >= 60 ? "Strong performance!" :
               avgMatchScore >= 40 ? "Getting there - room to grow" :
